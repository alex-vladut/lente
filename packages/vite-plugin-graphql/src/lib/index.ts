...
import loader from 'graphql-let/loader.js';
...
export default function viteGraphql(
  opts: Options = { rootContext: __dirname }
): PluginOption[] {
  ...
  const viteBabel: Plugin = {
    ...
    async transform(source, id, options) {
      ...
      const result = await babel.transformAsync(code, {
        ...babelOptions,
        root: projectRoot,
        filename: `${id}.tsx`, // it has to have the tsx extension for it to work (this is different from how @vite/plugin-react works)
...
viteGraphql.preambleCode = preambleCode;
